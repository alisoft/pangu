...
import { GENERATE_ROUTES_DYNAMIC, GET_INFO } from '@/store/modules/user/actions';

export function applyRouter(router: Router) {
  router.beforeEach(async to => {
    ...
    if (store.getters['user/allowRouters'] && store.getters['user/allowRouters'].length > 0) {
      ...
      const allowRouters = await store.dispatch(`user/${GENERATE_ROUTES_DYNAMIC}`, {
        router,
        info,
      });
      if (allowRouters) {
