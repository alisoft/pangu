...
const mixin = {
  ...
  mounted() {
    /* 获取url连接域名，用于多平台迁移*/
    const localUrl = window.location.href.slice(
      0,
      window.location.href.indexOf("/", 9) + 1
    );
    const socketUrl = localUrl.replace(/http|https/gi, "ws");
    // 读取.env 多坏境里的数据
    this.VITE_APP_IMAGE_URL_PRE = process.env.VITE_APP_BASE_URL;
    this.VITE_APP_BASE_URL = process.env.VITE_APP_BASE_URL;
    this.VITE_APP_BASE_WS_URL =
      process.env.VITE_APP_ENV === "development"
        ? process.env.VITE_APP_BASE_WS_URL
        : socketUrl;
    // 获取token和个人基本信息
    this.accessTokenMixin = getToken();
    this.userBaseInfoMixin = JSON.parse(localStorage.getItem("L_userBaseInfo"));
    /* 获取时间点*/
    this.todayTimeMixin = this.$momentMini()
      .startOf("day")
      .format("YYYY-MM-DD HH:mm:ss");
    this.currentTimeMixin = this.$momentMini(new Date()).format(
      "YYYY-MM-DD HH:mm:ss"
    );
    this.todayTimeMixinLast = this.$momentMini()
      .endOf("day")
      .format("YYYY-MM-DD HH:mm:ss");
    this.beforeThreeDateTimeMixin = this.$momentMini()
      .add(-3, "days")
      .format("YYYY-MM-DD HH:mm:ss");
    this.yesterdayTimeMixin = this.$momentMini()
      .add(-1, "days")
      .format("YYYY-MM-DD HH:mm:ss");
  },
  methods: {
