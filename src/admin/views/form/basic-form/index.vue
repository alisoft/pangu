  ...
  <page-container :title="t(`${route.meta.title}`)">
    ...
    <a-card :bordered="false">
      <a-form :label-col="labelCol" :wrapper-col="wrapperCol">
        <a-form-item
          :label="t('form.basicform.title.label')"
        ...
        <a-form-item
          :label="t('form.basicform.date.label')"
        ...
        <a-form-item
          :label="t('form.basicform.goal.label')"
          v-bind="validateInfos.goal"
        >
          <a-textarea
        ...
        <a-form-item
          :label="t('form.basicform.standard.label')"
          v-bind="validateInfos.standard"
        >
          <a-textarea
        ...
        <a-form-item
          :label="t('form.basicform.public.label')"
          v-bind="validateInfos.public"
        >
          <div>
            <a-radio-group v-model:value="modelRef.public">
              <a-radio value="1">{{
                t("form.basicform.radio.public")
              }}</a-radio>
              <a-radio value="2">{{
                t("form.basicform.radio.partially-public")
              }}</a-radio>
              <a-radio value="3">{{
                t("form.basicform.radio.private")
              }}</a-radio>
            </a-radio-group>
            <a-form-item
              :style="{ marginBottom: 0 }"
              v-bind="validateInfos.publicUsers"
            >
              <div
                :style="{
                  margin: '8px 0',
              ...
              >
                ...
                >
                  <a-select-option value="1">{{
                    t("form.basicform.option.A")
                  }}</a-select-option>
                  <a-select-option value="2">{{
                    t("form.basicform.option.B")
                  }}</a-select-option>
                  <a-select-option value="3">{{
                    t("form.basicform.option.C")
                  }}</a-select-option>
                </a-select>
        ...
        >
          <a-space>
            ...
            <a-button @click="saveFormData">{{
              t("form.basicform.form.save")
            }}</a-button>
          </a-space>
        </a-form-item>
      </a-form>
...
import type { ResponseBody } from "@/admin/api/typing";
import type { BasicFormResponse } from "@/admin/api/form/basic-form";
import { saveBasicFormData } from "@/admin/api/form/basic-form";
...
export default defineComponent({
  setup() {
    ...
    const rulesRef = reactive({
      title: [
        { required: true, message: () => t("form.basicform.title.required") },
      ],
      date: [
        { required: true, message: () => t("form.basicform.date.required") },
      ],
      goal: [
        { required: true, message: () => t("form.basicform.goal.required") },
      ],
      standard: [
        {
          required: true,
          message: () => t("form.basicform.standard.required"),
        },
      ],
    });
    ...
    const saveFormData = (): void => {
      ...
      localStorage.set(
        LOCAL_SAVE_BASIC_FORM_DATA_KEY,
