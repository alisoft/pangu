<template>
  ...
  <div class="mr-3 rowSS">
    ...
    >
      ...
      >
        <el-input
          v-model="searchFormMixin.errorLog"
          class="widthPx-150"
          placeholder="错误日志"
        />
      </el-form-item>
      ...
      >
        <el-input
          v-model="searchFormMixin.pageUrl"
          class="widthPx-150"
          placeholder="页面路径"
        />
      </el-form-item>
  ...
  >
    ...
    <el-table-column fixed="right" align="center" label="操作" width="80">
      <template #default="{ row }">
        <el-button type="text" size="small" @click="tableDelClick(row)"
          >删除</el-button
        >
      </template>
    </el-table-column>
  ...
  >
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button type="primary" @click="detailDialogMixin = false"
          >确 定</el-button
        >
      </span>
    </template>
...
let { pageNum, pageSize, handleCurrentChange, handleSizeChange } =
  tablePageHook(selectPageReq);
...
const multiDelBtnClick = async () => {
  ...
  await proxy.elConfirmMixin(
    "删除",
    `您确定要删除【${deleteNameTitle.slice(0, stringLength)}】吗`
  );
